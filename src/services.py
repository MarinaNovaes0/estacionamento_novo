...
def registrar_entrada_saida(cursor, conn, nome, placa, transacao):
    ...
    try:
        ...
        cursor.execute("SELECT ID FROM tb_cliente WHERE NOME = %s", (nome,))
        ...
        if transacao.lower() == "entrada":
            ...
            cursor.execute("""
                SELECT COUNT(*) 
                FROM tb_entrada_saida 
                WHERE ID_CARRO = %s AND ID_CLIENTE = %s AND DT_HORA_ENTRADA IS NULL AND DT_HORA_SAIDA IS NULL
            """, (carro_id, cliente_id))
            ...
            sql = """
                INSERT INTO tb_entrada_saida (ID_CARRO, ID_CLIENTE, DT_HORA_ENTRADA) 
                VALUES (%s, %s, %s)
            """
            valores = (carro_id, cliente_id, data_hora_atual)
            ...
            """
            valores = (data_hora_atual, carro_id)
        else:
...
def inserir_cliente_carro(cursor, conn, nome, telefone, placa, marca):
    ...
    try:
        ...
        sql_entrada_saida = "INSERT INTO tb_entrada_saida (ID_CARRO, ID_CLIENTE, DT_HORA_ENTRADA, DT_HORA_SAIDA) VALUES (%s, %s, NULL, NULL)"
